package pt.ulisboa.tecnico.cmov.pharmacist.util.placesautocomplete
...
import pt.ulisboa.tecnico.cmov.pharmacist.util.placesautocomplete.exception.InitializationException
import pt.ulisboa.tecnico.cmov.pharmacist.util.placesautocomplete.listener.OnPlacesDetailsListener
import pt.ulisboa.tecnico.cmov.pharmacist.util.placesautocomplete.model.Address
...
class PlaceAPI private constructor(
    ...
    internal fun autocomplete(input: String): ArrayList<Place>? {
        ...
        try {
            ...
            when (e) {
                is MalformedURLException -> logError(e, "Error connection to places api")
                is IOException -> logError(e, "Error connection to places api")
            }
            return resultList
        } finally {
            conn?.disconnect()
            ...
            try {
                ...
                when (e) {
                    ...
                    is MalformedURLException -> showDetailsError(
                        "Error processing to places API", listener, e
                    )
                    is IOException -> showDetailsError("Error processing to places API", listener, e)
                }
            } finally {
                conn?.disconnect()
            }
    ...
    private fun checkInitialization() {
        if (TextUtils.isEmpty(apiKey)) {
            throw InitializationException("Please initialize the api before using it")
        }
    }
    ...
    private fun logError(e: Exception, idk : String) {
        Log.e(TAG, idk, e)
    }
    ...
    private fun parseAutoCompleteData(jsonResults: StringBuilder): ArrayList<Place>? {
        ...
        try {
            ...
            when {
                ...
                else -> Log.e(TAG, "Cannot process JSON results", e)
            }
            return resultList
        }
    }
    ...
    private fun showDetailsError(resource: String, listener: OnPlacesDetailsListener, e: Exception) {
        logError(e, resource)
        resource.let { listener.onError(it) }
    }
    ...
    private fun parseDetailsError(
        ...
        if (errorJson.has(ERROR_MESSAGE)) {
            ...
            "Cannot process JSON results".let { listener.onError(it) }
        }
    }
